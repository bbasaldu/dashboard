...
const getPathColor = (label) => {
  const num = label[label.length - 1];
  return pathColors[+num - 1];
};
const pathColorsDark = pathColors.map((d) => d3.interpolateRgb(d, "#777")(0.3));
const getPathColorDark = (label) => {
  const num = label[label.length - 1];
  return pathColorsDark[+num - 1];
};
const monthValues = [
...
const getTickValues = (range) => {
  ...
  if (endDay < 30) {
    ...
    if (endDay < 15 && range[0].getMonth() !== range[1].getMonth()) {
      values.splice(end, 1);
    }
  }
  if (startDay > 15) {
    const start = values.findIndex((d) => d.getMonth() === range[0].getMonth());
    values.splice(start + 1, 1, range[0]);
  }
  return values;
};
  ...
  const setToolTips = (index) => {
    ...
    data.forEach((d, i) => {
      ...
      tooltip
        ...
        .style("color", getPathColorDark(d.label))
        .html(value);
    });
  };
    ...
    if (pointMarkerGroup.empty()) {
      ...
      data.forEach((d, i) => {
        const pointMarker = pointMarkerGroup
          ...
          .attr(
            "transform",
            `translate(${x}, ${yScale(d.pointData[index].y)})`
          );
        pointMarker
          .append("circle")
          .attr("fill", "none")
          .attr("stroke", getPathColorDark(d.label))
          .attr("stroke-width", 2)
...
export const changeData = (vars, data) => {
  ...
  if (vars.animate) {
    ...
    yAxis.call(d3.axisLeft(yScale).tickSize(tickSize).ticks(yTicks));
  }
  ...
  const handleExit = (exit) => {
    exit.each(function (d) {
      ...
      copyPath
        ...
        .attr("stroke-dashoffset", "unset");
      if (data.length !== 0) {
        copyPath.attr("d", (d) => line(d.pointData));
      }

      const copyPathLen = copyPath.node().getTotalLength();
      copyPath.remove();
      const rawPath = path.attr("d");
      path
        .transition()
        .duration(1000)
        .attr("data-len", copyPathLen)
        ...
        .attr("d", (d) => (data.length === 0 ? rawPath : line(d.pointData)))
        .remove();
      const tooltip = d3.select(`#${id}_tooltip_${d.label}`);
      tooltip.remove();
    });
...
export const rangeChange = (vars, data) => {
  const { id, yScale, yTicks, xScale, line, tickSize, height, theme, range } =
    vars;
  const svg = d3.select(`#${id}_svg`);
  ...
  grid
    ...
    .duration(1000)
    .call(
      xGridLines(xScale).tickValues(tickValues).tickSize(-height).tickFormat("")
    );
  grid.selectAll("line").attr("stroke", theme.second);
