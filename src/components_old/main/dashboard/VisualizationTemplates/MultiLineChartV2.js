  ...
  const calculateVariables = useCallback(
    (animate, data, range) => {
      ...
      const yMin = d3.min(data, (line) =>
          d3.min(line.pointData, (d) => d.y))
          const yMax = d3.max(data, (line) =>
          d3.max(line.pointData, (d) => d.y))
      //const showingData = data.filter(d => d.showing === true)
      //   const yMin = d3.min(showingData, (line) =>
      ...
      //     })
      //   );
      //   const yMax = d3.max(showingData, (line) =>
      //     d3.max(line.pointDat a, (d) => {
      //       if (d.x >= range[0] && d.x <= range[1]) {
      //         return d.y;
      //       }
      //     })
